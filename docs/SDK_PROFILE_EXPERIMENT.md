...
**CodeQL was not run.** It has Python data-flow and control-flow query libraries, but there is no equivalent SDK query, pinned CodeQL CLI or execution environment in this repository. Therefore no CodeQL accuracy or model-authoring time comparison is claimed. The next gate is a reproducible CodeQL query, independent annotation of real agent repositories, and measurement of framework model effort. Do not interpret this benchmark as evidence for a new language.
